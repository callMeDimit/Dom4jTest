...
package com.undergrowth.dom4j.util;
...
public abstract class StringUtils {
	 ...
	 * @param ignoreCase
	 *            是否忽略大小写
	 * @return true 包含指定字符串 false 不包含字指定符串
	 */
	public static boolean containStr(String[] strArr, String str,
			boolean ignoreCase) {
		String matchStr = str.trim();
		if (ignoreCase) {
			// 忽略大小写处理
			for (String string : strArr) {
				if (string.trim().equalsIgnoreCase(matchStr)) {
					return true;
				}
			}
		} else {
			// 不忽略大小写处理
			for (String string : strArr) {
				if (string.trim().equals(matchStr)) {
					return true;
				}
			}
		}
